...
function HighlightedLine({ line, lineNumber }: { line: string; lineNumber: number }) {
  ...
  return (
    <div className="flex">
      <span className="text-[#8a8a95] select-none mr-4 w-6 text-right inline-block">
        {lineNumber}
      </span>
      <span>
        {tokens.map((token, i) => (
...
function CodeBlock({ code }: { code: string }) {
  ...
  return (
    <div className="relative bg-[#0d0d14] border border-[#1a1a25] rounded-lg overflow-hidden">
      {/* Copy button */}
      <button
        onClick={handleCopy}
      ...
      >
        {copied ? (
          ...
          <Copy size={14} className="text-[#8a8a95]" aria-hidden="true" />
        )}
      </button>
...
function QuickStartCard({ icon, title, description, buttonText, href, delay }: QuickStartCardProps) {
  ...
  return (
    <motion.div
      ref={ref}
      className="bg-[#111118] border border-[#1a1a25] p-8 hover:border-[#333340] transition-colors"
      initial={{ opacity: 0, y: 30 }}
      animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }}
      transition={{ duration: 0.5, delay }}
    >
      <div className="text-[#8a8a95] mb-6" aria-hidden="true">
        {icon}
      </div>
      <h3 className="font-mono text-sm tracking-[0.2em] text-[#e8e8e8] mb-3">
        {title}
      </h3>
      <p className="text-sm text-[#8a8a95] leading-relaxed mb-6">
        {description}
      </p>
      <a
        href={href}
        className="inline-block font-mono text-xs tracking-[0.15em] text-[#8a8a95] hover:text-[#e8e8e8] transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#d4a843]"
      >
        {buttonText} &rarr;
      </a>
    </motion.div>
...
function ArchitectureItem({ title, description, delay }: ArchitectureItemProps) {
  ...
  return (
    <motion.div
      ref={ref}
      className="border-l-2 border-[#1a1a25] pl-6 py-2"
...
export default function DocsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-50 focus:px-4 focus:py-2 focus:bg-[#d4a843] focus:text-[#0a0a0f] focus:font-mono focus:text-sm"
      >
      ...
      <section ref={heroRef} className="relative py-24 px-6 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          <motion.p
            className="font-mono text-sm tracking-[0.2em] text-[#8a8a95]"
            initial={{ opacity: 0, y: 20 }}
            animate={isHeroInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
      ...
      <main id="main-content">
        ...
        <section id="sdk" className="py-20 px-6 bg-[#08080c]" aria-labelledby="code-example-heading">
          <div className="max-w-4xl mx-auto">
            ...
            <p className="mt-6 text-center font-mono text-xs text-[#8a8a95] tracking-[0.1em]">
        ...
        <section id="contracts" className="py-24 px-6 bg-[#08080c]" aria-labelledby="cta-heading">
          <div className="max-w-2xl mx-auto text-center">
            ...
            <p className="font-mono text-sm text-[#8a8a95] mb-10">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link
                href="/register"
                className="inline-block px-10 py-4 border border-[#e8e8e8] text-[#e8e8e8] font-mono text-sm tracking-[0.2em] transition-all hover:bg-[#e8e8e8] hover:text-[#0a0a0f] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#d4a843] focus-visible:ring-offset-2 focus-visible:ring-offset-[#08080c]"
              >
                GET STARTED
              </Link>
              <a
                ...
                className="inline-block px-10 py-4 border border-[#333340] text-[#8a8a95] font-mono text-sm tracking-[0.2em] transition-all hover:border-[#8a8a95] hover:text-[#e8e8e8] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#d4a843] focus-visible:ring-offset-2 focus-visible:ring-offset-[#08080c]"
              >
                VIEW ON GITHUB
              </a>
            </div>
